...
async function handleRequest(request, env) {
    ...
    if (method === 'POST' && path === '/api/command') {
      ...
      const VALID = ['setFaceRecognition','setAutoConfirm','setBoomerang','setSmileShutter','setArFilters','setSlideshow','setSendQrEmails'];
      if (!eventId || !action || !VALID.includes(action)) {
      ...
      cmds.push({ id, eventId, action, value: !!value, boothId, createdAt: new Date().toISOString() });
